...
void LogS(const std::string& author, std::string format, ...)
{
	...
	if (!LogSile.is_open())
	{
		...
		LogSile.open(WORKDIR + "Logs\\" + Time::startDateAndTime + ".txt");
	}
	...
	LogSile.write(fullMsg, fullMsg.size());
	va_end(vl);
}

void LogS(const std::string& author, const char* format, ...)
{
